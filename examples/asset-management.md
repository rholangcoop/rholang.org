---
title: Managing Ressources (Content)
filepath: rcat.rho
filetype: vue
order: 2
---
...
new metaDataMapStore, songMapStore, userMapStore, remunerate, testLog(`rho:io:stderr`) in {
  ...
  contract @["Ressources", "store"](@songDataIn, @songMetadataIn, songIdOut) = {
  ...
  contract @["Ressources", "retrieveSong"](@songIdIn, songDataOut) = {
    for (@metaDataMap <- metaDataMapStore; @songMap <- songMapStore) {
  ...
  contract @["Ressources", "retrieveMetaData"](metaDataMapOut) = {
  ...
  contract @["Ressources", "newUserId"](userIdOut) = {
    new userId in {
  ...
  contract @["Ressources", "play"](@songIdIn, @userIdIn, permittedOut) = {
  ...
  contract @["Ressources", "playCount"](@userIdIn, countOut) = {
    for (@userMap <- userMapStore) {
      userMapStore!(userMap) |
