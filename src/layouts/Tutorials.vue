    ...
    <div class="container flex flex-align-top">
      <div class="sidebar">
        <template v-if="links" v-for="(group, i1) in links" >
          ...
          <template v-for="(item, i2) in group.items">
            <g-link :exact="item.link == '/tutorials/'" class="menu-item menu-link" :to="item.link" :key="`link-${i1}-${i2}`">
              {{ item.title }}
            </g-link>
          </template>
        </template>
      ...
      <div v-if="subtitles.length > 0 && subtitles[0].depth !== 3" class="sidebar sidebar--right hide-for-small">
        ...
        <ul v-if="subtitles.length" class="menu-item submenu">
          <li class="submenu__item" :class="'submenu__item-depth-' + subtitle.depth" v-for="(subtitle,index) in subtitles" :key="subtitle.value + index">
...
export default {
  ...
  computed: {
    ...
    editLink () {
      ...
      if((path.match(new RegExp("/", "g")) || []).length == 1) path = path + '/introduction'
